...
class Graf():
    ...
    def za_shrambo(self):
        '''pripravi trenutne podatke za shranjevanje v zgodovino'''
        vs = []
        ps = []
        for vozlisce in self.vozlisca:
            koordinate = self.canvas.coords(vozlisce)
            vs.append((vozlisce, *sredisce(koordinate)))
        for povezava in self.povezave:
            vrednosti = self.povezave[povezava]
            ps.append(tuple(vrednosti))
        return (vs, ps)
...
class GrafEditor():
    def __init__(self, master, width=500, height=500):
        ...
        self.canvas.bind('<Control-z>', self.razveljavi)
    ...
    def razveljavi(self, event):
        ...
        nova_vozlisca = {}
        for (id, x, y) in vs:
            v = self.graf.dodaj_vozlisce(x, y)
            nova_vozlisca[id] = v
        for (v1, v2) in ps:
            self.graf.dodaj_povezavo(nova_vozlisca[v1], nova_vozlisca[v2])
